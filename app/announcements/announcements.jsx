  ...
  return (
    <div className="min-h-screen">
      ...
      {announcements.length === 0 ? (
        <div className="text-center py-16 sm:py-24 bg-white rounded-2xl border-2 border-slate-200 shadow-sm">
...
function AnnouncementCard({ announcement }) {
  const { label, createdAt, deadline, attachments } = announcement;
  ...
  const isExpiringSoon = daysRemaining !== null && daysRemaining >= 7 && daysRemaining <= 14;

  return (
    <motion.div initial="hidden" animate="visible" variants={fadeIn}>
      <div className="bg-white border-2 border-slate-200 rounded-2xl hover:shadow-lg hover:border-slate-300 transition-all duration-300 relative overflow-hidden group/card">
        {/* Corner Ribbon */}
        {(isUrgent || isExpiringSoon) && (
          <div className={`absolute top-0 right-0 z-10 ${isUrgent ? 'pulse-animation' : ''}`}>
            <div className={`relative ${isUrgent ? 'bg-red-500' : 'bg-amber-500'} text-white px-10 py-2 text-xs font-bold transform rotate-45 translate-x-7 translate-y-3 shadow-lg flex items-center justify-center`}>
              {isUrgent ? 'URGENT' : 'EXPIRING'}
            </div>
          </div>
        )}
        ...
        <div className="p-3 sm:p-4 pb-2 sm:pb-3">
          ...
          <div className="mb-4">
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 sm:gap-3">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 font-semibold rounded-lg border border-blue-200 text-xs sm:text-sm w-fit">
              ...
              {deadline && (isUrgent || isExpiringSoon) && (
                <>
                  <span className="hidden sm:inline text-slate-300">•</span>
                  <div className={`inline-flex items-center gap-2 px-3 py-1.5 font-semibold rounded-lg border text-xs sm:text-sm w-fit ${isUrgent
          ...
          {attachments && attachments.length > 0 && (
            <AttachmentsSection attachments={attachments} isExpanded={isExpanded} setIsExpanded={setIsExpanded} />
          )}
        </div>

        {/* Bottom Arrow Toggle - Only show if there are attachments */}
        {attachments && attachments.length > 0 && (
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="w-full flex items-center justify-center py-3 border-t border-slate-200 hover:bg-slate-50 transition-colors group/btn"
...
function AttachmentsSection({ attachments, isExpanded, setIsExpanded }) {
  ...
  return (
    <div className={isExpanded ? "pt-3 border-t border-slate-200" : ""}>
      <AnimatePresence>
        {isExpanded && (
          <motion.div
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-900 truncate group-hover:text-blue-700" title={fileName}>
                        {fileName}
                      </div>
          ...
          {attachments.slice(0, 2).map((attachment, idx) => {
            ...
            return (
              ...
              >
                <FileIcon extension={file?.extension} isUrl={isUrl} />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-900 truncate" title={fileName}>
                    {fileName}
                  </div>
                  <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                    <span>{fileType}</span>
